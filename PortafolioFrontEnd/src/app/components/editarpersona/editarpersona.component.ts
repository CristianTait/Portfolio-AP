...
export class EditarpersonaComponent {
  ...
  ngOnInit() {
    ...
    this.http.get<any>(`https://portafoliobackend-xekr.onrender.com/personas/detail/${personaID }`).subscribe(
      (response) => {
  ...
  onSubmit() {
    ...
    const personaID = +this.route.snapshot.params['id'];
    this.http.put<any>(`https://portafoliobackend-xekr.onrender.com/personas/update/${personaID}`, this.personas).subscribe(
      () => {
