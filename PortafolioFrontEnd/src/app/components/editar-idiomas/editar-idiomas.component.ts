...
export class EditarIdiomasComponent {
  ...
  ngOnInit() {
    ...
    this.http.get<any>(`https://portafoliobackend-xekr.onrender.com/idiomas/detail/${idiomaId}`).subscribe(
      (response) => {
  ...
  onSubmit() {
    ...
    const idiomaId = +this.route.snapshot.params['id'];
    this.http.put<any>(`https://portafoliobackend-xekr.onrender.com/idiomas/update/${idiomaId}`, this.idiomas).subscribe(
      () => {
