...
export class EditarexperiencialaboralComponent {
  ...
  ngOnInit() {
    ...
    this.http.get<any>(`http://localhost:8080/explab/detail/${expLabId}`).subscribe(
      (response) => {
  ...
  onSubmit() {
    ...
    const expLabId = +this.route.snapshot.params['id'];
    this.http.put<any>(`http://localhost:8080/explab/update/${expLabId}`, this.experiencias).subscribe(
      () => {
