...
export class EditproyectocomponenteComponent {
  ...
  ngOnInit() {
    ...
    this.http.get<any>(`http://localhost:8080/proyectos/detail/${proyectoId}`).subscribe(
      (response) => {
  ...
  onSubmit() {
    ...
    const proyectoId = +this.route.snapshot.params['id'];
    this.http.put<any>(`http://localhost:8080/proyectos/update/${proyectoId}`, this.proyecto).subscribe(
      () => {
