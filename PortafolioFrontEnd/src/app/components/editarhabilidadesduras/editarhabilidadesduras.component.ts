...
export class EditarhabilidadesdurasComponent {
  ...
  ngOnInit() {
    ...
    this.http.get<any>(`http://localhost:8080/habduras/detail/${HDId}`).subscribe(
      (response) => {
  ...
  onSubmit() {
    ...
    const HDId = +this.route.snapshot.params['id'];
    this.http.put<any>(`http://localhost:8080/habduras/update/${HDId}`, this.skillsDuras).subscribe(
      () => {
