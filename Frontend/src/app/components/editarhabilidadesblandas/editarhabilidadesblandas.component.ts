...
export class EditarhabilidadesblandasComponent {
  ...
  ngOnInit() {
    ...
    this.http.get<any>(`http://localhost:8080/habblandas/detail/${HBId}`).subscribe(
      (response) => {
  ...
  onSubmit() {
    ...
    const HBId = +this.route.snapshot.params['id'];
    this.http.put<any>(`http://localhost:8080/habblandas/update/${HBId}`, this.skills).subscribe(
      () => {
